...
import { UpdateLineItemBody, LineItemData, LineItemDetail, toLineItemDetail } from '../schemas/dto/LineItems/line-items.dto.js';
	...
	try {
		...
		const created = await Promise.all(
			(items || []).map(async (it) => {
				...
				const menu_item_id = (it as any).menu_item_id && typeof (it as any).menu_item_id === 'object' ? (it as any).menu_item_id.menu_item_id : (it as any).menu_item_id;
		...
		const rows = await Promise.all(
			created.map(async (id) => await prisma.line_items.findUnique({ where: { line_item_id: id }, include: lineItemsDefaultInclude }))
		);
...
export async function updateLineItem(line_item_id: string, data: UpdateLineItemBody): Promise<LineItemDetail> {
	try {
		const updated = await prisma.line_items.update({ where: { line_item_id }, data, include: lineItemsDefaultInclude });
