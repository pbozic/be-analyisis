import type { Prisma } from '@prisma/client';
import { client as stripe } from '../../lib/stripe.js';
import prisma from '../prisma.js';
import type { MODULE_TYPE } from '../schemas/interfaces.ts';
import { RESERVATION_ACTIONS, ALL_ACTIONS } from '../../constants/actions.ts';

const MODULE: MODULE_TYPE = 'reservations';
const FLATTENED_ACTIONS: { name: string; module: MODULE_TYPE }[] = Object.entries(ALL_ACTIONS).flatMap(
	([module, actions]) =>
		Object.values(actions).map((name) => ({
			name,
			module: module as MODULE_TYPE,
		}))
);

type ActionLimit = {
	action: keyof typeof RESERVATION_ACTIONS;
	limit: number | null;
};

type SeedActionBundle = {
	name: string;
	price_cents: number;
	seed_key: string;
	actions: ActionLimit[];
};

const ACTION_BUNDLES: SeedActionBundle[] = [
	// ...same as in your original seeder...

	{
		name: 'Basic',
		price_cents: 500,
		seed_key: 'action_bundle_basic_reservations',
		actions: Object.keys(RESERVATION_ACTIONS).map((key) => ({
			action: key as keyof typeof RESERVATION_ACTIONS,
			limit: 0,
		})),
	},
	{
		name: 'Free Trial',
		price_cents: 0,
		seed_key: 'action_bundle_trial_reservations',
		actions: [
			{ action: 'MANAGE_BOOKING', limit: null },
			{ action: 'ADD_EMPLOYEE', limit: 1 },
			{ action: 'ADD_LOCATION', limit: 1 },
			{ action: 'ADD_SERVICE', limit: null },
			{ action: 'ADD_CUSTOMER', limit: 1 },
			{ action: 'SEND_SMS', limit: 160 },
			{ action: 'VIEW_DASHBOARD', limit: null },
		],
	},
	{
		name: 'Standard',
		price_cents: 1000,
		seed_key: 'action_bundle_standard_reservations',
		actions: [
			{ action: 'MANAGE_BOOKING', limit: null },
			{ action: 'ADD_EMPLOYEE', limit: 1 },
			{ action: 'ADD_LOCATION', limit: 1 },
			{ action: 'ADD_SERVICE', limit: null },
			{ action: 'ADD_CUSTOMER', limit: 1 },
			{ action: 'SEND_SMS', limit: 160 },
			{ action: 'VIEW_DASHBOARD', limit: null },
		],
	},
	{
		name: 'Premium',
		price_cents: 2000,
		seed_key: 'action_bundle_premium_reservations',
		actions: [
			{ action: 'MANAGE_BOOKING', limit: null },
			{ action: 'ADD_EMPLOYEE', limit: 1 },
			{ action: 'ADD_LOCATION', limit: 1 },
			{ action: 'ADD_SERVICE', limit: null },
			{ action: 'ADD_CUSTOMER', limit: 1 },
			{ action: 'SEND_SMS', limit: 160 },
			{ action: 'VIEW_DASHBOARD', limit: null },
		],
	},
];

const ADDONS: {
	name: string;
	price_cents: number;
	seed_key: string;
	actions: ActionLimit[];
}[] = [
	// ...same as in your original seeder...
	{
		name: 'Additional SMS',
		price_cents: 6,
		seed_key: 'addon_sms_reservations',
		actions: [{ action: 'SEND_SMS', limit: 1 }],
	},
	{
		name: 'Additional Employee',
		price_cents: 1000,
		seed_key: 'addon_employee_reservations',
		actions: [{ action: 'ADD_EMPLOYEE', limit: 1 }],
	},
	{
		name: 'Additional Location',
		price_cents: 1000,
		seed_key: 'addon_location_reservations',
		actions: [{ action: 'ADD_LOCATION', limit: 1 }],
	},
];

async function getOrCreateProduct(name: string, type: 'bundle' | 'addon'): Promise<string> {
	console.log('Getting or creating product for:', name, type);

	const productName = type === 'bundle' ? `Klikni Reservation Bundle: ${name}` : `Klikni Reservation Addon: ${name}`;

	// Search products by name
	const searchResult = await stripe.products.search({
		query: `name:'${productName}' AND active:'true'`,
		limit: 1,
	});

	if (searchResult.data.length > 0) {
		return searchResult.data[0]!.id;
	}

	// Create if not found
	const created = await stripe.products.create({
		name: productName,
		metadata: { type, name },
	});
	return created.id;
}

async function getOrCreatePrice(productId: string, priceCents: number, seedKey: string): Promise<string> {
	console.log('Getting or creating price for product:', productId, priceCents, seedKey);

	// Search prices by product and seed_key
	const prices = await stripe.prices.list({ product: productId, limit: 100 });
	const found = prices.data.find((p) => p.metadata?.seed_key === seedKey && p.unit_amount === priceCents);

	if (found) return found.id;

	const created = await stripe.prices.create({
		currency: 'eur',
		unit_amount: priceCents,
		recurring: { interval: 'month' },
		product: productId,
		metadata: { seed_key: seedKey },
	});

	return created.id;
}

export default async function seedActionBundlesWithStripe(): Promise<void> {
	console.log('Seeding action bundles and addons with Stripe integration...');
	await prisma.$transaction(
		async (tx: Prisma.TransactionClient) => {
			// 1. Create actions
			for (const act of FLATTENED_ACTIONS) {
				await tx.action.upsert({
					where: { name: act.name },
					update: { module: { set: act.module } },
					create: {
						name: act.name,
						module: act.module,
					},
				});
			}
			console.log('All actions upserted.');

			const actionsMap: Record<string, { action_id: string }> = {};
			const allActions = await tx.action.findMany();
			for (const act of allActions) {
				actionsMap[act.name] = { action_id: act.action_id };
			}

			// 2. Seed action_bundles with Stripe product/price
			console.log('Seeding action bundles...');
			for (const bundle of ACTION_BUNDLES) {
				console.log('Seeding bundle:', bundle.name);
				const productId = await getOrCreateProduct(bundle.name, 'bundle');
				const stripePriceId = await getOrCreatePrice(productId, bundle.price_cents, bundle.seed_key);

				const createdBundle = await tx.action_bundle.upsert({
					where: { name: bundle.name },
					update: {
						price_cents: bundle.price_cents,
						stripe_price_id: stripePriceId,
						stripe_product_id: productId,
						module: { set: MODULE },
					},
					create: {
						name: bundle.name,
						price_cents: bundle.price_cents,
						stripe_price_id: stripePriceId,
						stripe_product_id: productId,
						module: MODULE,
					},
				});

				await tx.action_bundle_action.deleteMany({
					where: { action_bundle_id: createdBundle.action_bundle_id },
				});

				for (const sa of bundle.actions) {
					const actionName = RESERVATION_ACTIONS[sa.action];
					const foundAction = actionsMap[actionName];
					if (!foundAction) throw new Error(`Action not found: ${actionName}`);

					await tx.action_bundle_action.create({
						data: {
							action_bundle: { connect: { action_bundle_id: createdBundle.action_bundle_id } },
							action: { connect: { action_id: foundAction.action_id } },
							limit: sa.limit,
							module: MODULE,
						},
					});
				}
			}

			// 3. Seed addons with Stripe product/price
			console.log('Seeding addons...');
			for (const addon of ADDONS) {
				console.log('Seeding addon:', addon.name);
				const productId = await getOrCreateProduct(addon.name, 'addon');
				const stripePriceId = await getOrCreatePrice(productId, addon.price_cents, addon.seed_key);

				const createdAddon = await tx.addon.upsert({
					where: { name: addon.name },
					update: {
						price_cents: addon.price_cents,
						stripe_price_id: stripePriceId,
						stripe_product_id: productId,
						module: { set: MODULE },
					},
					create: {
						name: addon.name,
						price_cents: addon.price_cents,
						stripe_price_id: stripePriceId,
						stripe_product_id: productId,
						module: MODULE,
					},
				});

				await tx.addon_action.deleteMany({
					where: { addon_id: createdAddon.addon_id },
				});

				for (const aa of addon.actions) {
					const actionName = RESERVATION_ACTIONS[aa.action];
					const foundAction = actionsMap[actionName];
					if (!foundAction) throw new Error(`Action not found: ${actionName}`);

					await tx.addon_action.create({
						data: {
							addon: { connect: { addon_id: createdAddon.addon_id } },
							action: { connect: { action_id: foundAction.action_id } },
							limit: aa.limit,
							module: MODULE,
						},
					});
				}
			}
		},
		{
			maxWait: 5000, // 5 seconds max wait to connect to prisma
			timeout: 20000, // 20 seconds
		}
	);
}

seedActionBundlesWithStripe()
	.then(() => {
		console.log('Seeding complete.');
		process.exit(0);
	})
	.catch((err) => {
		console.error('Seeding failed:', err);
		process.exit(1);
	});
