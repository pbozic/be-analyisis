...
async function getCoordinates(user) {
	...
	const uniqueAddresses = [...new Set(addressVariations)].filter(
		(addr) =>
			addr &&
			addr !== ', Slovenia' &&
			addr !== 'Slovenia' &&
	...
	for (let i = 0; i < uniqueAddresses.length; i++) {
		...
		try {
			...
			if (data.length > 0) {
				const bestResult =
					data.find((r) => r.type === 'house' || r.type === 'building' || r.type === 'address') || data[0];
...
async function main() {
	try {
		console.log('Starting geocoding process...', __dirname);
		const data = fs.readFileSync(__dirname + '/usersBefore.json', 'utf-8');
		...
		console.log('\n=== Geocoding completed ===', newUsers.length);
		fs.writeFileSync(__dirname + '/usersAfter.json', JSON.stringify(newUsers, null, 2));
