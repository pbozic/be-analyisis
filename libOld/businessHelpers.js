...
import { SCORING_POINTS_REASON } from './constants.js';
import BusinessUserDao from '../dao/BusinessUsers.js';
import { SocketStore } from '../socket.js';
/**
 * Reorders menu categories and items for each business based on manual order arrays.
 * For each menu: sorts categories using menu_categories_ordered, then sorts items in each category using menu_items_ordered.
 * Also resolves sides/extras to full menu items to avoid circular references.
 * @param {object[]} businesses - List of businesses with menus and categories.
 * @returns {object[]} The same list with menus/categories/items ordered and enriched.
 */
		...
		if (business.menus) {
			...
			business.menus.forEach((menu) => {
				if (menu.menu_categories_ordered) {
					const orderedCategoryIds = menu.menu_categories_ordered;
					menu.categories.sort((a, b) => {
						return (
							orderedCategoryIds.indexOf(a.menu_category_id) -
							orderedCategoryIds.indexOf(b.menu_category_id)
						);
					});
				} else {
...
/**
 * Finds a menu item inside a menu by its menu_item_id.
 * @param {object} menu - A menu object with categories and menu_items.
 * @param {string} itemId - The menu_item_id to locate.
 * @returns {object|null} The found menu item, or null if not present.
 */
...
/**
 * Aggregates unlinked late events into scoring_points as penalties per business/user pair.
...
async function aggregateAutoRejectedOrders() {
	...
	for (const order of recentOrders) {
		if (Array.isArray(order.timeline) && order.timeline.some((event) => event.status === 'AUTO_REJECTED')) {
	...
	for (const [business_id, N_autorejected] of Object.entries(grouped)) {
		let penaltyPoints = Math.floor(N_autorejected / 3);
		while (penaltyPoints > 0) {
			await prisma.scoring_points.create({
				data: {
					businesses: { connect: { business_id: business_id } },
					points: 1,
					isPenalty: true,
...
/**
 * Joins all users of a business into a socket.io room.
 * @param {string} businessId - The business id.
 * @param {string} roomName - The socket room name.
 * @returns {Promise<void>}
 */
async function joinAllBusinessUsersToRoom(businessId, roomName) {
	const busers = await BusinessUserDao.getBusinessUsersByBusinessId(businessId);
	await Promise.all(busers.map((buser) => SocketStore.addUserToRoom(buser.user_id, roomName)));
}
export { reorderMenusAndCategories };
