 ...
 */
export function toScheduleSlotWithBookings(row: ScheduleSlotWithBookingsPrisma): any {
	const r = row;

	const booking_slots = (r.booking_slots || []).map((bs) => ({
	...
	const schedule_slot_exceptions = (r.schedule_slot_exceptions || []).map((ex) => ({
