...
export function toBusinessClientWithBusinessResponse(row: BusinessClientWithBusinessPrisma): BusinessClientWithBusinessResponse {
...
export function toBusinessClientWithOrdersResponse(row: BusinessClientWithOrdersPrisma): BusinessClientWithOrdersResponse {
    const r = row as BusinessClientWithOrdersPrisma;
    const asRec = r as Record<string, any>;

    const taxi_orders = asRec.taxi_orders
        ? (asRec.taxi_orders as any[]).map((o) => ({ order_id: o.order_id, status: o.status, created_at: toIso(o.created_at) }))
        : null;
