import { MenuItemResponseSchema, MenuItemsResponseSchema, DailyMealMenuBaseSchema, MenuBaseSchema, MenuItemVersionResponseSchema } from './menu.dto.js';
...
export function toMenuItemResponse(payload: MenuItemPayload): MenuItemResponse {
    const item = {
        ...
        menu_category: (payload as { menu_category?: { menu_category_id?: string; menu_id?: string; names?: unknown; menu_items_ordered?: unknown; menu_categories_categories?: unknown } }).menu_category
            ? {
                  menu_category_id: (payload as { menu_category?: { menu_category_id?: string } }).menu_category?.menu_category_id,
                  ...
                  menu_items_ordered: (payload as { menu_category?: { menu_items_ordered?: unknown } }).menu_category?.menu_items_ordered,
                  menu_categories_categories: (payload as { menu_category?: { menu_categories_categories?: unknown } }).menu_category?.menu_categories_categories,
              }
            : undefined,
        documents: Array.isArray((payload as { documents?: unknown }).documents)
            ? ((payload as { documents?: unknown }).documents as Array<unknown>).map((d) => ({
                  ...
                  files: Array.isArray((d as { files?: unknown }).files) ? ((d as { files?: unknown }).files as Array<unknown>).map((f) => ({ file_id: (f as { file_id?: string }).file_id, url: (f as { url?: string }).url, document_id: (f as { document_id?: string }).document_id })) : undefined,
              }))
            : undefined,
    };
...
export function toMenuCategoryResponse(category: CategoryPayload) {
    return {
        ...category,
        menu_items: Array.isArray((category as { menu_items?: unknown }).menu_items) ? ((category as { menu_items?: unknown }).menu_items as Array<MenuItemPayload>).map(toMenuItemResponse) : undefined,
...
export function toMenuResponse(payload: MenuPayload): MenuBase {
    const out = {
        ...
        created_at: toIso((payload as { created_at?: unknown }).created_at),
        updated_at: toIso((payload as { updated_at?: unknown }).updated_at),
        categories: Array.isArray((payload as { categories?: unknown }).categories) ? ((payload as { categories?: unknown }).categories as Array<CategoryPayload>).map(toMenuCategoryResponse) : undefined,
    };
    ...
    const converted = {
        ...
        categories: Array.isArray((payload as { categories?: unknown }).categories)
                        ? ((payload as { categories?: unknown }).categories as Array<CategoryPayload>).map((c) => ({
                                    ...c,
                                    menu_items: Array.isArray((c as { menu_items?: unknown }).menu_items) ? ((c as { menu_items?: unknown }).menu_items as Array<MenuItemPayload>).map(toMenuItemResponse) : undefined,
                                    menu_categories_categories: (c as { menu_categories_categories?: unknown }).menu_categories_categories,
                            }))
            : undefined,
    };
...
export function toMenuItemList(payload: MenuItemPayload[]) {
    const normalized = payload.map((p) => ({ ...p, created_at: toIso((p as { created_at?: unknown }).created_at), updated_at: toIso((p as { updated_at?: unknown }).updated_at) }));
