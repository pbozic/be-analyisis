 ...
 *
 * Source: schemas/dto/Business/business.mappers.ts:386
...
describe('Business Mapper - toCrmModuleResponse', () => {
	it('should map Prisma data to DTO and validate against schema', async () => {
		...
		try {
			const { toCrmModuleResponse } = await import('./business.mappers.js');
			const result = toCrmModuleResponse(mockData as any);

			expect(result).toBeDefined();
			expect(typeof result).toBe('object');
			console.log('✅ toCrmModuleResponse passed schema validation');
		} catch (error: any) {
