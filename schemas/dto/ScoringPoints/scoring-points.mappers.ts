...
export function toScoringPointsDetail(payload: ScoringPointsWithIncludesPrisma | null | undefined): ScoringPointsDetail {
    const p = payload;
    return ScoringPointsDetailSchema.parse({
        ...
        late_events: Array.isArray(p.late_events) ? p.late_events.map((le: any) => ({ late_events_id: le.late_events_id })) : [],
    });
}
