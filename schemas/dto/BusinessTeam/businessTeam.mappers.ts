import { BusinessTeamResponseSchema, BusinessTeamWithUsersResponseSchema, MinimumUserSchema } from './businessTeam.dto.js';
...
export function toBusinessTeamWithUsersResponse(row: BusinessTeamWithUsersPrisma): BusinessTeamWithUsersResponse {
    ...
    const users = Array.isArray(r.users)
        ? r.users.map((u: any) => MinimumUserSchema.parse({ user_id: u.user_id, first_name: u.first_name ?? null, last_name: u.last_name ?? null }))
        : null;
