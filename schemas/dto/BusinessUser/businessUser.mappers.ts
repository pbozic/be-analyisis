import {
    ...
    BusinessUserWithBusinessResponseSchema, BusinessUserCreationResponseSchema
...
import type { BusinessUserWithBusinessPrisma, BusinessUserDefaultPrisma } from '../../../prisma/includes/businessUsers.js';
...
export function toBusinessUserWithBusinessResponse(row: BusinessUserWithBusinessPrisma): BusinessUserWithBusinessResponse {
    const r = row as Record<string, any>;
...
export function toBusinessUsersList(rows: BusinessUserDefaultPrisma[] | BusinessUserWithBusinessPrisma[]): BusinessUserListResponse {
    return (rows as any[]).map((r) => toBusinessUserResponse(r));
}

export function toBusinessUserCreationResponse(user: any, businessUser: BusinessUserDefaultPrisma): BusinessUserCreationResponse {
    return BusinessUserCreationResponseSchema.parse({ user, businessUser: toBusinessUserResponse(businessUser) });
}

export default { toBusinessUserResponse, toBusinessUserWithBusinessResponse, toBusinessUsersList, toBusinessUserCreationResponse };
