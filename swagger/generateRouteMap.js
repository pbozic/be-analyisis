	...
	if (fs.existsSync(apiPath)) {
		...
		lines.forEach((line, index) => {
			...
			if (useMatch) {
				...
				const nestedRoutesMap = buildRoutesMap(
					nestedRouterPath,
					routesPath,
					workspacePath,
					nestedBasePath,
			...
			if (routeMatch) {
				...
				console.log(
					`Found route: ${method.toUpperCase()} ${routePath} -> ${controllerName}.${functionName} at ${controllerPath}:${functionLine}, documented: ${isDocumented}`
				);
