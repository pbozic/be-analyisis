...
async function notifyUpcomingCreditExpirations() {
	try {
		...
		const oneDayWarnings = await WalletFundsDao.findCreditsExpiringInDays(1);
...
async function transferReservedCreditsForOrder(userId,destination_acc,amount,orderId,orderType) {
	try {
		...
		const reservedCredits = await WalletFundsDao.getReservedCredits(userId,orderId,orderType);
