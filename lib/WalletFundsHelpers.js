...
const { FUNDS_TYPE } = require("./constants");

async function reserveAvailableWalletFundsForOrder(userId,amount,orderId, orderType = "order") {
	try {
		if(amount<=0){
		...
		for(let i=0;i<included_wallet_funds.length;i++){
			...
			const reservedWF = await WalletFundsDao.reserveFunds(included_wallet_funds[i].wallet_funds_id,split_amount,orderId, orderType)
...
async function releaseReservedWalletFundsForOrder(userId,orderId, amount=-1) {
	try {
		...
		while(remaining_release_amount>0){
			await WalletFundsDao.releaseFunds(reservedCredits[i].wallet_funds_id, Math.min(remaining_release_amount,reservedCredits[i].amount))
...
async function transferReservedWalletFundsForOrder(userId,destination_acc,amount,orderId,service_type) {
	try {
		if(amount<=0){
			throw new Error("Amount must be greater than 0.");
		}
		...
		for(let i=0;i<included_wallet_funds.length;i++){
			...
			const subtractedWF = await WalletFundsDao.subtractFunds(included_wallet_funds[i].wallet_funds_id,split_amount,orderId,service_type)
...
async function reserveCreditsForOrder(userId, amount, orderId, credit_type, orderType = "order"){
	try {
		...
		for(let i=0;i<availableCredits.length;i++){
			...
			const reservedCredit = await WalletFundsDao.reserveFunds(availableCredits[i].wallet_funds_id,split_amount,orderId, orderType)
...
async function transferReservedCreditsForOrder(userId,destination_acc,amount,orderId,service_type) {
	try {
		if(amount<=0){
			throw new Error("Amount must be greater than 0.");
		}

		const reservedCredits = await WalletFundsDao.getReservedCredits(userId,orderId);
		...
		for(let i=0;i<reservedCredits.length;i++){
			...
			const subtractedCredit = await WalletFundsDao.subtractFunds(reservedCredits[i].wallet_funds_id,split_amount,orderId,service_type)
			if(fundsToSubtract<=0){
				break
			}
		}
