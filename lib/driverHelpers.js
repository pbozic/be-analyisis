   ...
   let allDrivers = [...deliveryDrivers, ...drivers];
    for (let driver of allDrivers) {
        ...
        if (driver.last_ping_at < new Date(new Date() - 20 * 60 * 1000)) {
            ...
            await OneSignal.sendNotificationToUser("Klikni Driver", "You are now offline, please open KlikniApp", driver.user.one_signal_id);
            continue;
          
        }
            // send notification to driver
         if (driver.last_ping_at < new Date(new Date() - 10 * 60 * 1000)) {
            // send notification to driver
            //TODO: send notification to driver
            await OneSignal.sendNotificationToUser("Klikni Driver", "You are inactive, please open KlikniApp", driver.user.one_signal_id);
         }
    };
