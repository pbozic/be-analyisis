...
function scoreWordBuy(price) {
    const baseScore = 1;
    const scaleFactor = 10; // Controls how fast the score increases
    return baseScore + Math.log(1 + price / scaleFactor) * 10; // Logarithmic scaling
}

// Function to calculate promo_section_buy score based on tier
function scorePromoSection(tier) {
    const tierScores = { "T1": 3, "T2": 2, "T3": 1 }; // Higher tier gets higher score
