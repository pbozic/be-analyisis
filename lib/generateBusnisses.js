...
const { log } = require("@prisma/sdk/dist/logger");
const FoodByCuisine = {
	Italian: {
...
async function generateAIImages(restaurantName) {
	return "https://processed-model-result.s3.us-east-2.amazonaws.com/94605724-88b6-439b-82bc-7ac270372c00_0.png";
...
async function generateMenuItem(axios, category, menu, cuisine, business_id, user_id) {
	let mainIngredient = faker.helpers.arrayElement([faker.food.meat(), faker.food.vegetable()]);
	...
	let image = await getFoodImage(name);
...
async function generateBusiness() {
	...
	for (let cuisine of categoryNames) {
		let category = await generateMenuCategory(axiosC, cuisine, menu.menu_id, business.business_id);
		const itemCount = faker.number.int({ min: 5, max: 15 });
		// create menu items
		for (let j = 0; j < itemCount; j++) {
			let item = generateMenuItem(axiosC, category, menu, cuisine, business.business_id, userB.user_id);
		}
	}
