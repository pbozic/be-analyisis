...
/**
 * Generate a templated food description using faker for variety.
 * @param {string} mainIngredient - Main ingredient name.
 * @param {string} dish - Dish name.
 * @param {string} cuisine - Cuisine name.
 * @returns {string} Generated description.
 */
function generateFoodDescription(mainIngredient, dish) {
	const templates = [
...
/**
 * Retrieve an image URL for a given food type from Pexels (or use a predefined fallback).
 * @param {string} food_type - Food type or dish name.
...
/**
 * Create a menu item via API for a given category and menu, uploading a generated image.
 * @param {object} axios - Axios instance with baseURL and auth.
 * @param {object} category - Menu category entity.
 * @param {object} menu - Menu entity.
 * @param {string} cuisine - Cuisine label.
 * @param {string} business_id - Business ID.
 * @param {string} user_id - Author user ID.
 * @returns {Promise<object>} Created menu item response data.
 */
async function generateMenuItem(axios, category, menu, cuisine, business_id, user_id) {
	...
	let description = generateFoodDescription(mainIngredient, name);
...
/**
 * Get a random location inside a polygon.
 * @param {object} polygon - GeoJSON polygon.
 * @returns {{latitude:string,longitude:string}} Random coordinate inside polygon.
 */
...
/**
 * Download an image and return its base64 string.
 * @param {string} url - Image URL.
 * @returns {Promise<string>} Base64 encoded image (no prefix).
 */
...
/**
 * Generate a full business with categories and items using the public API.
 * Also generates and uploads images and marks phone as verified.
...
async function generateBusiness() {
	...
	for (let cuisine of categoryNames) {
		...
		for (let j = 0; j < itemCount; j++) {
			await new Promise((resolve) => globalThis.setTimeout(resolve, 1000));
			await generateMenuItem(axiosC, category, menu, cuisine, business.business.business_id, userB.user_id);
		}
	}
...
let translations = {};
/**
 * Translate a string into multiple target locales using ARML Translator, with memoization.
 * @param {string} text - Source text in English.
 * @returns {Promise<object>} Map of locale => translated text.
 */
...
/**
 * Generate N businesses asynchronously.
 * @param {number} num - Number of businesses to generate.
 * @returns {Promise<void>}
 */
