...
export async function findSlots({
	...
	for (const slot of scheduleSlots) {
		...
		while (current.clone().add(totalDurationMinutes, 'minute').isSameOrBefore(slotEnd)) {
			...
			// Must be inside at least one booking slot range
			const insideBookingSlot = slot.booking_slots.some((bs: BookingSlot) => {
				const bsStart = moment(bs.start_time);
				const bsEnd = moment(bs.end_time);
				return current.isSameOrAfter(bsStart) && current.isBefore(bsEnd);
			});
			if (!insideBookingSlot) {
