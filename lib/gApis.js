...
async function getDistanceForRoute(points, mode = 'driving') {
	...
	let waypoints;
	if (points.length > 2) {
		waypoints = points.slice(1, -1).map((p) => `${p.latitude},${p.longitude}`);
	}
	try {
		...
		if (!route || !route.legs?.length) {
			console.error('No route found or invalid API response', response.data);
...
export { addressFromCoordinates };
export { getDistanceForRoute };
export { getDistanceForRouteV2 };
export default {
	apiKey,
	client,
