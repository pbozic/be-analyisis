...
async function sendOrderNotifications(user, driver, user_id, driver_id, status) {
	...
	if (user_id && notifications[status].user !== null) {
		return await sendNotificationToUser(l10nTextHeadingUser.taxi, notifications[status].user, user_id);
	}
...
async function sendDeliveryOrderNotifications(user, driver, user_id, driver_id, status) {
	...
	const l10nTextHeadingUser = getLocalisedTexts("HEADING", user);
	const l10nTextHeadingDriver = getLocalisedTexts("HEADING", driver);
	//TODO: implement all the different notifications for user/driver
	const notifications = {
		MERCHANT_ACCEPTED: {
			user: l10nTextUser.accepted,
			driver: l10nTextDriver.accepted
		},
	...
	if (user_id && notifications[status]?.user && notifications[status].user !== null) {
		return await sendNotificationToUser(l10nTextHeadingUser.delivery, notifications[status]?.user, user_id);
	}
	console.log("DELIVERY DRIVER NOTIFICATION_OUT", driver_id, status);
	if (driver_id && notifications[status]?.user && notifications[status].user !== null) {
		console.log("DELIVERY DRIVER NOTIFICATION", notifications[status]?.driver);

		const driver = await getDriverById(driver_id);
		if (driver && notifications[status]?.driver !== null) {
			return await sendNotificationToUser(l10nTextHeadingDriver.delivery, notifications[status]?.driver, driver.user_id);
		} else {
			const delivery_driver = await getDeliveryDriverById(driver_id);
			if (delivery_driver && notifications[status]?.driver !== null) {
				return await sendNotificationToUser(l10nTextHeadingDriver.delivery, notifications[status]?.driver, delivery_driver.user_id);
			}
		}
	}
	return false;
