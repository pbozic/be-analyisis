...
async function sendOrderNotifications(user, driver, user_id, driver_id, status) {
	...
	if (driver_id) {
		...
		if (driver && notifications[status].driver !== null) {
			await sendNotificationToUser(l10nTextHeadingDriver.taxi, notifications[status].driver, driver.user_id);
		}
	}
}
/**
...
async function sendDeliveryOrderNotifications(user, driver, user_id, driver_user_id, status) {
	...
	if (driver_user_id && notifications[status] && notifications[status].driver !== null) {
		await sendNotificationToUser(l10nTextHeadingDriver.delivery, notifications[status]?.driver, driver_user_id);
	}
}
/**
 * Send credit expiration notification to user.
 *
 * @param {object} user
 * @returns {Promise<void>}
 */
async function sendCreditExpirationNotifications(user) {
	...
	if (user?.user_id) {
		...
		console.log('No user ID for credit expiration notification');
	}
}
/**
 * Send referral notification to user.
 *
 * @param {object} user
 * @returns {Promise<void>}
 */
async function sendReferralNotifications(user) {
	...
	if (user?.user_id) {
		...
		console.log('No user ID for referral notification');
	}
}
export { sendOrderNotifications };
export { sendDeliveryOrderNotifications };
export { sendCreditExpirationNotifications };
export { sendReferralNotifications };
export default {
