...
async function checkAccountActive(accountId) {
    try {
        ...
        return account.requirements.currently_due.length===0//account.charges_enabled && account.payouts_enabled;
    } catch (error) {
...
async function checkAccountState(accountId) {
    try {
        ...
        console.info(`Charges Enabled: ${isAccountActive}`);
        console.info(`Requirements:`, requirements);
        console.info(`Capabilities:`, capabilities);

        // Return account state information
        return {
            id: account.id,
            chargesEnabled: isAccountActive,
