...
const {RESTAURANT_FEE, SERVICE_TYPE } = require('./constants');
...
async function createSplitPayment(customer_acc, restaurant_acc, order_id, payment_method,total_price, merchant_cut, return_url, type = "order_payment"){//transfer_prices, transfer_destinations){
    try{
        const paymentIntent = await stripe.paymentIntents.create({
...
async function createSplittablePayment(customer_acc, order_id, payment_method, total_price, order_type, cuts, return_url, type = "order_payment"){//transfer_prices, transfer_destinations){
    try{
        const paymentIntent = await stripe.paymentIntents.create({
            amount: Math.round(total_price),
            currency: "EUR",
...
/**
 * Create a PaymentIntent on Stripe and store in DB
 */
...
/**
 * Validate Stripe PaymentIntent before using it to create an order
 */
