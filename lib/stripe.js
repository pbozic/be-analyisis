...
async function createSplittablePayment(customer_acc, order_id, payment_method, total_price, order_type, cuts, return_url, type = "order_payment", capture_method='automatic'){//transfer_prices, transfer_destinations){
    try{
        const paymentIntent = await stripe.paymentIntents.create({
            amount: Math.round(total_price),
            currency: "EUR",
