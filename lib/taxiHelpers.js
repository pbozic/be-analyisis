...
async function selectTaxiOrderDrivers(order) {
	try {
		...
		let vehicleFilters = {
			//TODO: change private driver filtering logic to exclude cargo van drivers (SEDAN is a temp fix)
			class: order.type === ORDER_TYPE.VEHICLE_TRANSFER_COMBO ? vehicleClass === "SEDAN"
				: (vehicleClass === VEHICLE_CLASS.ANY ? "" : vehicleClass === VEHICLE_CLASS.PRIVATE_DRIVER ? "SEDAN" : vehicleClass),
