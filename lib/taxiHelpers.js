...
async function selectTaxiOrderDrivers(order) {
	...
	if (drivers.length > 0) {
		for (let draw of drivers) {
			let isSent = await TaxiOrderDao.isOrderSent(order.order_id, draw);
...
async function checkIfOrdersNeedSending() {
	try {
		...
		const timeThreshold = new Date(currentTime - 5 * 60 * 1000); // 5 minutes ago

		// console.log("Current time:", currentTime);
		// console.log("Time threshold (150 minutes ago):", timeThreshold);
		...
		let orders = await TaxiOrderDao.getOrders({
			where: {
				...
				OR: [
					...
					{ last_sent_at: { lte: timeThreshold } }, // Include records where last_sent_at is less than or equal to 5 minutes ago
				],
				is_scheduled: false,
			}
		});
		console.log("Open taxi orders not scheduled:", orders.length);
		const eightHoursLater = new Date(); // Current date and time
		eightHoursLater.setHours(eightHoursLater.getHours() + 0.5); // Add 8 hours
		console.log("Scheduled time threshold:", eightHoursLater);
