...
async function findTaxiOrderDrivers(order) {
	return new Promise(async (resolve, reject) => {
		...
		if (order?.driver?.driver_id) {
			...
			drivers = [await DriverDao.getDriverById(order.driver_id)];
		} else {
...
async function selectTaxiOrderDrivers(order) {
	...
	// if (drivers.length === 0) {
	// 	console.taxiHelpers("No drivers found within the radius. Using fallback option.");
	// 	drivers = await prisma.drivers.findMany();
	// }
    console.taxiHelpers("before check", drivers);
...
async function sendTaxiOrderToDriver(order, driver) {
	...
	sendNotificationToUser("New taxi order", "You have a new taxi order", driver.user_id);
