...
const { TAXI_ORDER_STATUS, VEHICLE_CLASS, ORDER_TYPE, TAXI_STARTING_FEE, COST_PER_KM, VEHICLE_CATEGORY,
	DRIVE_FEE, CARGO_TRANSFER_FEE, FUNDS_TYPE, SERVICE_TYPE
} = require("./constants");
...
async function handlePaymentRefund(order_id,user_id,payment, additional_workers, vehicle_class, skip_fee=false){
	...
	if(transfer_amount_credits>0){
		await WalletFundsHelpers.transferReservedCreditsForOrder(user_id, "platform", transfer_amount_credits, order_id, SERVICE_TYPE.TAXI)
	}
	if(payment.type==="WALLET" && transfer_amount_regular>0){
		await WalletFundsHelpers.transferReservedWalletFundsForOrder(user_id, "platform", transfer_amount_regular, order_id, SERVICE_TYPE.TAXI)
	}
