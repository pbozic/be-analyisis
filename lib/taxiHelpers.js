...
const { TAXI_ORDER_STATUS, VEHICLE_CLASS } = require("./constants");
...
async function selectTaxiOrderDrivers(order) {
	try {
		...
		let vehicleFilters = {
			//TODO: change private driver filtering logic to exclude transfer drivers (SEDAN is a temp fix)
			class: (vehicleClass === VEHICLE_CLASS.ANY ? "" : vehicleClass === VEHICLE_CLASS.PRIVATE_DRIVER ? "SEDAN" : vehicleClass),
