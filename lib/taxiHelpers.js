...
async function calculateTransferRidePrice(route, vehicleCategory) {
	...
	const { totalDistanceKM, totalDurationS,totalDurationM } = await gApi.getDistanceForRoute(route.map(loc=>loc.coordinates), "driving");
	if (!totalDistanceKM) {
	...
	const totalPrice = basePrice + (pricePerKm * totalDistanceKM);
