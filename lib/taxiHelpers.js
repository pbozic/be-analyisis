...
const { TAXI_ORDER_STATUS, VEHICLE_CLASS, ORDER_TYPE, TAXI_STARTING_FEE, COST_PER_KM } = require("./constants");
...
async function calculateTransferRidePrice(startLocation, endLocation, vehicleCategory) {
	if (vehicleCategory !== VEHICLE_CLASS.STANDARD && vehicleCategory !== VEHICLE_CLASS.PREMIUM) {
		throw new Error("Invalid vehicle category. Only STANDARD and PREMIUM are allowed.");
	}

	const { distanceM } = await gApi.distanceBetweenTwoPoints(startLocation, endLocation, "driving", new Date());
	if (!distanceM) {
	...
	let priceSurge = getPriceSurgeDataForTransferTrip(startLocation, endLocation, new Date(), null, null);

	const finalPrice = totalPrice * priceSurge.multiplier;
