...
async function scheduledOrdersNotificationsHandler() {
	try {
		const now = new Date();
		const twentyFourHoursLater = moment(now).add(24, "hours").startOf("minute").toDate();
		const twentyFourHoursLaterEnd = moment(twentyFourHoursLater).endOf("minute").toDate();
		const oneHourLater = moment(now).add(1, "hours").startOf("minute").toDate();
		const oneHourLaterEnd = moment(oneHourLater).endOf("minute").toDate();

		console.log("SCHEDULED TIME: ", twentyFourHoursLater, oneHourLater)
