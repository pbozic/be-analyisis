...
async function findTaxiOrderDrivers(order) {
	...
	return new Promise(async (resolve, reject) => {
		...
		if (order?.driver?.driver_id) {
			...
			drivers = [await DriverDao.getDriverById(order.driver.driver_id)];
		} else {
...
async function selectTaxiOrderDrivers(order) {
	try {
		...
		let radius = 2000;
		let vehicleFilters = {
			class: order.preferences?.vehicle_class || null,
			category: order.preferences?.vehicle_category || null
		}
