...
const { TAXI_ORDER_STATUS, VEHICLE_CLASS, ORDER_TYPE, TAXI_STARTING_FEE, COST_PER_KM, VEHICLE_CATEGORY,
	DRIVE_FEE, CARGO_TRANSFER_FEE, FUNDS_TYPE
} = require("./constants");
...
async function selectTaxiOrderDrivers(order) {
	try {
		...
		while (drivers.length === 0 && radius < 30000) {
			let fetchedDrivers = await prisma.drivers.inRadius(order.pickup_location.coordinates, radius, order.preferences.ride_requirements, vehicleFilters);
			console.log("in radius", fetchedDrivers?.length);
			const onlineDrivers = fetchedDrivers.filter(async driver => {
				console.log("driver.user_id", driver.user_id, UserSockets.get(driver.user_id));
				...
				if (numPendingOrders >= MAX_PENDING_ORDERS) return false;
		...
		console.log("Eligible drivers: ", eligibleDrivers.map(d => d.driver_id));	
...
async function calculateTransferRidePrice(route, vehicleCategory) {
	...
	const { totalDistanceM, totalDurationS,totalDurationM } = await gApi.getDistanceForRoute(route.map(loc=>loc.coordinates), "driving");
	if (!totalDistanceM) {
...
async function handlePaymentRefund(order_id,user_id,payment, additional_workers, vehicle_class, skip_fee=false){
	...
	//refund credits first, handle the remaining amount later.
	const refund_amount = Math.round(total_price_cents * (skip_fee ? 1 : 0.9))//TODO: Make cancellation fee a constant
	const refund_credits_amount = Math.min(credit_discount_used,refund_amount)
	const refund_amount_after_credits = Math.max(Math.round(refund_amount-refund_credits_amount), 0)
	console.info("payment refund logs\n",
		payment,
		refund_amount,
		refund_credits_amount,
		refund_amount_after_credits,
	)
	await WalletFundsHelpers.releaseReservedWalletFundsForOrder(user_id, order_id, refund_credits_amount)

	//calculate transfered credits and if needed, create new ones with fresh expiry dates
	if(refund_credits_amount > credits_wf_amount_reserved){
		await WalletFundsDao.createCredit({
			user: { connect: { user_id: user_id } },
			amount: refund_credits_amount - credits_wf_amount_reserved,
	...
	if(PAYMENT_TYPE==="WALLET"){
		...
		const reserved_wf_refund_amount = Math.min(refund_amount_after_credits,regular_wf_amount_reserved)
		console.log(refund_amount_after_credits,regular_wf_amount_reserved)
		await WalletFundsHelpers.releaseReservedWalletFundsForOrder(user_id, order_id, Math.min(refund_amount_after_credits,regular_wf_amount_reserved))
		if(payment.status="PAID" && refund_amount_after_credits > regular_wf_amount_reserved){
			await WalletFundsDao.createWalletFunds(user_id, refund_amount_after_credits - regular_wf_amount_reserved,null)
		}
	}
		...
		if(payment.payment_intent_id){
			...
			if(![ 'succeeded', 'canceled', 'processing'].includes(payment_intent.status)){
				...
				const refund_obj = await stripe.client.refunds.create({
					amount: refund_amount_after_credits,
