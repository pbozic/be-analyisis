...
async function scheduledOrdersNotificationsHandler() {
	try {
		...
		const twentyFourHoursLaterEnd = new Date(twentyFourHoursLater.getTime() + 60*1000);
		const oneHourLater = new Date(now.getTime() + 60*60*1000);
		oneHourLater.setSeconds(0, 0);
		const oneHourLaterEnd = new Date(oneHourLater.getTime() + 60*1000);
		...
		let startTimestamp = twentyFourHoursLater.getTime()-offset;
		let endTimestamp = twentyFourHoursLaterEnd.getTime()-offset;
		...
		startTimestamp = oneHourLater.getTime()-offset;
		endTimestamp = oneHourLaterEnd.getTime()-offset;
		const HourOrders = scheduled.filter(o => {
			const departureTime = o.preferences?.departure_time ? new Date(o.preferences.departure_time).getTime() : null;
			return departureTime && departureTime >= startTimestamp && departureTime <= endTimestamp;
		});
		console.log("DAY", DayOrders)
		console.log("HOUR", HourOrders)
		for (let order of DayOrders) {
			...
			await sendNotificationToUser(l10nTextHeading?.scheduled_tomorrow, l10nText?.scheduled_tomorrow + new Date(order.preferences.departure_time).toLocaleTimeString("sl-SI", {timeZone: "Europe/Ljubljana"}), order.user_id);
		}
...
async function checkIfOrdersNeedSending() {
	try {
		...
		// tresholdAmountOfTimeLater.setMinutes(tresholdAmountOfTimeLater.getMinutes()); // Add 30 min
