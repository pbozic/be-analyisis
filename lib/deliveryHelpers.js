...
async function checkIfRestaurantOrderIsPrepared() {
	...
	for (const or of orders) {
		const businessType = or.business?.type;
		const readyTime = new Date(or.details?.ready_for_pickup_at);
		// readyTime.setMinutes(readyTime.getMinutes() + (businessType === BUSINESS_TYPE.LOCAL ? 10 : 0));
		if (
			readyTime.getTime() <= now.getTime() &&
			[BUSINESS_TYPE.RESTAURANT, BUSINESS_TYPE.LOCAL].includes(businessType)
		) {
...
async function autoRejectDeliveryOrders() {
	...
	for (const or of orders) {
		...
		if (or.created_at.getTime() > threshold_time.getTime()) {
			continue;
		}
...
const verifyOrderCosts = async (orderData) => {
	...
	//minimum_order_fee
	const expectedFee = merchant_business?.minimum_order - orderTotal;
	const actualFee = orderData.details.minimum_order_fee;
	console.info('Comparing:', actualFee, expectedFee);
	const checkMinimumFee = orderTotal < merchant_business?.minimum_order;
	if (checkMinimumFee && actualFee.toFixed(2) !== expectedFee.toFixed(2)) return false;

	const expected_total_sum = delivery_cost + discountTotal + orderTotal + (checkMinimumFee ? expectedFee : 0);
...
function enrichItem(orderItem, menuItemLookup) {
	...
	if (orderItem.replaced_item && !orderItem.replaced_item.is_removed) {
 ...
 *
 * Removed items (`is_removed === true`) are skipped entirely and thus removed from the result.
 *
...
function validateAndEnrichOrderItems(orderItems, menuItemLookup) {
	...
	for (const orderItem of orderItems) {
		if (orderItem.is_removed) continue;
...
function calculateItemTotal(item) {
	function getDiscountedUnitPrice(price = 0, discount = 0) {
		return price * (1 - discount);
	}

	function computeCappedTotal(item, baseTotal) {
		if (item.replaced_item) {
			const replaced = item.replaced_item;
			const replacedUnit = (replaced.price ?? 0) * (1 - (replaced.discount ?? 0));
			const replacedQty = replaced.quantity ?? 1;

			const priceCap = replaced.is_weighted
				? WEIGHTED_ITEM_QTY_INCREASE_LIMIT * replacedQty * replacedUnit
				: replacedQty * replacedUnit;

			return Math.min(baseTotal, priceCap);
		} else if (item.is_weighted && typeof item.old_quantity === 'number') {
			const maxQty = WEIGHTED_ITEM_QTY_INCREASE_LIMIT * item.old_quantity;
			const unitPrice = Number(item.price ?? 0) / 1000; // grams → kg
			const discount = Number(item.discount ?? 0);
			const cappedTotal = unitPrice * (1 - discount) * maxQty;

			return Math.min(baseTotal, cappedTotal);
		}
	...
	const discountedUnitPrice = getDiscountedUnitPrice(unitPrice, discount);
	const baseItemTotal = discountedUnitPrice * quantity;
	const baseItemDiscount = unitPrice * discount * quantity;

	const finalItemTotal = computeCappedTotal(item, baseItemTotal);
	const finalItemDiscount = baseItemDiscount;
	...
	return {
		line_total: finalItemTotal + extrasTotal + sidesTotal,
		line_discount: finalItemDiscount + extrasDiscount + sidesDiscount,
	};
}
...
async function calculateAndVerifyPriceForOrderItems(order) {
	...
	const total_price = grand_total + discount_total + delivery_cost + (order.details?.minimum_order_fee ?? 0);
