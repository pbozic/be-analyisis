...
async function addDeliveryEarningToOrder(order, driver) {
	...
	const deliveryEarning = pricePerKm * order.details.distance;
	if (!order.details) {
		order.details = {};
	}
...
async function sendActiveOrdersToDeliveryDriver(driver) {
	try {
		...
		// Separate daily meal orders from other orders
		const dailyMealOrders = activeOrders.filter(order => order.is_daily_meal);
		const otherOrders = activeOrders.filter(order => !order.is_daily_meal);

		// Get the scheduled route excluding the first and last index (pickup locations)
		const scheduledRoute = driver?.scheduled_meals_route.slice(1, -1) || [];

		// Sort daily meal orders according to the scheduled route
		const sortedDailyMeals = scheduledRoute.map(routeStop => {
			return dailyMealOrders.find(order => order.delivery_location?.address === routeStop);
		}).filter(order => order); // Remove any undefined entries if no match was found

		// Combine sorted daily meal orders with the other orders
		const combinedOrders = [...sortedDailyMeals, ...otherOrders];

		// Send orders to the driver
		if (UserSockets.get(driver.user_id)) {
			console.info("Sending [active] orders to delivery driver: ", driver.user_id);
			UserSockets.get(driver.user_id).emit("load_active_orders__delivery", combinedOrders);
		}
	} catch (error) {
...
function generateItemsFromPreferences(preferences, menuItem) {
	...
	const createItem = (id, type) => ({
		menu_item_id: id,
		names: {
			en: type === 'substitution' ? 'substitution' : 'normal'
		},
		price: menuItem.price,
		discount: menuItem.discount
	});

	for (let i = 0; i < normal.amount; i++) {
		items.push(createItem(i, 'normal'));
	}

	const offset = normal.amount;
	for (let i = 0; i < substitution.amount; i++) {
		items.push(createItem(offset + i, 'substitution'));
	}
