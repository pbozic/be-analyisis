...
const verifyOrderCosts = async (orderData) => {
	...
	console.info("Comparing:",Number(orderData.details.minimum_order_fee), Number(merchant_business?.minimum_order) - Number(orderTotal))
	if(
		orderTotal < merchant_business?.minimum_order
		&& Number(orderData.details.minimum_order_fee) !== Number(merchant_business?.minimum_order) - Number(orderTotal)
	) return false
