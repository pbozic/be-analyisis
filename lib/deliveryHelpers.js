...
async function autoRejectDeliveryOrders() {
	...
	for (const or of orders) {
		...
		if (or.created_at.getTime() > threshold_time.getTime()) {
			if (type !== BUSINESS_TYPE.LOCAL || now.getTime() < new Date(or.scheduled?.time?.slice(0, -1)?.getTime())) {
				continue;
			}
		}
...
const verifyOrderCosts = async (orderData) => {
	...
	for (const item of order_items) {
		...
		if (item.is_weighted) {
			...
			quantity *= 1.2; // increase price by 20 percent to allow for possible quantity increase.
		}
