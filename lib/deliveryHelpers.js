...
import { haversineDistance, sortObjectsByNearestNeighbor } from './helpersLib.js';
...
function CalculateOrderDetails(provider, cartItems, selectedAddress, type = 'delivery', paymentType = 'cash') {
	let orderTotal = 0;
	let discountTotal = 0;
	...
	if (!student_meal) {
		...
		const pickupAddress = provider?.delivery_address || provider?.address;
		...
		const deliveryResult = calculateDistanceDelivery(
			{
				lat: deliveryLat,
				lng: deliveryLng,
			},
		...
		if (deliveryResult) {
			delivery_cost = deliveryResult.cost;
		}
	}
