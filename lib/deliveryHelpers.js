...
async function resendPendingOrdersToDeliveryDriver(driver) {
	try {
		...
		for (let sentOrder of sentOrders) {
			...
			if (!(order.delivery_driver_id || order.driver_id)) {
				if (UserSockets.get(driver.user_id)) {
...
async function calculateDeliveryOrderPaymentCuts(order) {
	...
	const reservedCredits = await WalletFundsDao.getReservedCredits(user_id, order_id, 'DELIVERY') || [];
...
const calculateOrderDeliveryCost=(orderData)=>{
	if (orderData?.details?.type === "delivery") {
		const distance = haversine(
			{
				lat: orderData.delivery_location?.coordinates?.latitude,
				lng: orderData.delivery_location?.coordinates?.longitude,
			},
			{
				lat: orderData.pickup_location?.coordinates?.longitude,
				lng: orderData.pickup_location?.coordinates?.longitude,
			}
		)
		let pricePer500 = 0.5;
		const delivery_cost = (Math.round(distance / 500) * pricePer500)
...
const verifyOrderCosts = async (orderData) => {
	...
	let delivery_cost = calculateOrderDeliveryCost(orderData);
	if(delivery_cost !== orderData.details.delivery_cost) return false

	const menu_item_ids = Array.isArray(orderData?.items) ? orderData.items.map(item=>item.menu_item_id) : []
	const merchant_business = await BusinessDao.getBusinessById(orderData.business_id)
	...
	if(
		orderTotal !== orderData.details.sub_total_price
		|| orderData.details.discount_savings !== discountTotal
	) return false
	...
	if(
		...
		&& orderData.details.minimum_order_fee !== merchant_business?.minimum_order - orderTotal
	) return false
