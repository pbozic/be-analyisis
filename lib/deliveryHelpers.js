...
const { DELIVERY_ORDER_STATUS, TAXI_ORDER_STATUS, DRIVE_FEE, RESTAURANT_FEE } = require("./constants");
...
async function checkIfRestaurantOrderIsPrepared() {
	...
	for (const order of orders) {
		const order = await DeliveryOrderDao.updateOrderStatus(order.order_id,DELIVERY_ORDER_STATUS.MERCHANT_READY_FOR_PICKUP)
...
async function resendPendingOrdersToDeliveryDriver(driver) {
	try {
		const sentOrders = await DeliveryOrderDao.getAlreadySentOrdersByDeliveryDriverId(driver.delivery_driver_id || driver.driver_id);
		for (let sentOrder of sentOrders) {
			const order = await DeliveryOrderDao.getOrder(sentOrder.order.order_id,{include:{user:{select:cropped_user_columns}}});
...
async function sendActiveOrdersToDeliveryDriver(driver) {
	try {
		const activeOrders = await DeliveryOrderDao.getActiveOrdersByDeliveryDriverId(driver.delivery_driver_id || driver.driver_id);
