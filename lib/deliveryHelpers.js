...
const { DELIVERY_ORDER_STATUS, TAXI_ORDER_STATUS, DRIVE_FEE, RESTAURANT_FEE, AUTOREJECT_THRESHOLD_MINUTES,
	SCORING_POINTS_REASON, FUNDS_TYPE, DELIVERY_COST_CALCULATION, DELIVERY_ORDER_END_STATES, SERVICE_TYPE
} = require("./constants");
...
async function selectDeliveryOrderDrivers(order) {
	...
	for (let driver of onlineDrivers) {
		...
		let { distance, duration, result } = await gApi.distanceBetweenTwoPoints(driver.location.coordinates, order.pickup_location.coordinates, "driving", new Date(), "best_guess");
...
async function revokeDeliveryOrderFromDrivers(order_id) {
	...
	for (let sent of deliveryOrderSent) {
		...
		SocketStore.removeUserFromRoom(sent.delivery_driver?.user_id,`order_${order_id}`)
			...
			if (shouldReleaseFunds(order, service_type, date_threshold)) {
				for (const wf of groupedReservedWFs[orderId]) {
					releasePromises.push(
						WalletFundsDao.releaseFunds(wf.wallet_funds_id, wf.amount)
							.catch(err => {
