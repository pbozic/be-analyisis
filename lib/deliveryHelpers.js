...
function calculateItemTotal(item) {
	...
	function getCappedPrice(item) {
		...
		if (item.is_weighted) {
			...
			const capPrice = capQty * unitPrice * WEIGHTED_ITEM_QTY_INCREASE_LIMIT;
			basePrice = Math.min(basePrice, capPrice);
			// console.log('capQty', capQty, capPrice, basePrice);
		}

		const thisItemPrice = basePrice * (1 - discount);

		if (item.replaced_item) {
			const replacedPrice = getCappedPrice(item.replaced_item);
			return Math.min(thisItemPrice, replacedPrice);
		}
	...
	const fullUnitPrice = item.is_weighted ? unitPrice / 1000 : unitPrice;

	const finalItemTotal = getCappedPrice(item);

	const fullItemPriceBeforeCap = fullUnitPrice * quantity;
	const baseItemDiscount = fullItemPriceBeforeCap - finalItemTotal;
	...
	return {
		...
		line_discount: baseItemDiscount + extrasDiscount + sidesDiscount,
	};
}
