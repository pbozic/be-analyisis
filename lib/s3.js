...
const GetObject = async (key, public) => {
...
const isObjectPublic = async (bucket, key) => {
  try {
      ...
      return "private"; // Assume private if an error occurs
  }
};
const SaveObject = async (key, data, mimeType, owners = {}, file, public = false) => {
...
async function updateS3ACLForPublicFiles() {
    ...
    for (const file of publicFiles) {
        try {
            console.log(`🌍 Updating S3 ACL for: ${file.key} in ${file.bucket}...`);
            ...
            console.log(`✅ Successfully updated ${file.key} to public-read.`);
        } catch (error) {
