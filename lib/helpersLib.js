...
function calculateBusinessEarnings(orders, business) {
    if (orders && orders.length > 0) {
        const totalEarnings = orders.reduce((sum, order) => {
            return sum + parseFloat(order.items.price || 0);
        }, 0);
    ...
    if (completedOrders && completedOrders.length > 0) {
        ...
        completedOrders.forEach(order => {
            ...
            if (timestamp) {
                ...
                const orderAmount = type === "TAXI_COMPLETED"
                    ? (parseFloat(order.payment.price) || 0)
                    : ((parseFloat(order.items.price) - parseFloat(order.details.delivery_cost)) || 0);
                if (order.order_id === "fc42a696-0cb0-48ab-8a15-160781385d01") {
