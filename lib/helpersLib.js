...
function calculateBusinessEarnings(orders, business) {
    if (orders && orders.length > 0) {
        ...
        return {
            ...
            distance_travelled: distance_travelled !== '' ? distance_travelled.trim() : '0 km',
            time_travelled: time_travelled !== '' ? time_travelled.trim() : '0h 0m',
...
function calculateDriversEarnings(driverOrders, driver) {
    if (driverOrders && driverOrders.length > 0) {
        ...
        const distanceTimeTravelled = formatDistanceTimeTravelled(distance, hours, minutes);
        console.log("distanceTimeTravelled: ", distanceTimeTravelled[0], ' & ', distanceTimeTravelled[1]);
        const [distance_travelled, time_travelled] = distanceTimeTravelled.split('/');
