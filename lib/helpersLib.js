...
const buildWhereObject = (queryWhere) => {
    ...
    if (queryWhere.preferences) {
        ...
        if (adults !== undefined) {
            where.preferences = { ...where.preferences, adults: parseInt(adults) };
        }
        if (children_under_140 !== undefined) {
            where.preferences = { ...where.preferences, children_under_140: parseInt(children_under_140) };
        }
        if (fixedPrice !== undefined) {
            where.payment = { ...where.payment, price: { equals: parseFloat(fixedPrice) } };
        }
        if (isAbove !== undefined) {
            where.payment = { ...where.payment, price: { gt: parseFloat(isAbove) } };
        }
        if (isBelow !== undefined) {
            where.payment = { ...where.payment, price: { lt: parseFloat(isBelow) } };
        }
    }
    if (queryWhere.rideRequirements) {
        where.preferences = { ...where.preferences, ride_requirements: Object.keys(queryWhere.rideRequirements).filter(req => queryWhere.rideRequirements[req]) };
    }
...
function calculateTotalEarnings(completedOrders, type, delivery_driver = false) {
