...
router.post('/login', async function (req, res, next) {
  console.log("login");
  let postData = jsonParse(req.headers["postdata"]);
  console.log(postData);

  let email = postData["email"];
  let password = postData["password"];

  let isInputValid = validateUserInput([email, password]);
  if (!isInputValid) {
    ...
    try {
      ...
      console.log("U:", dbResponse);

      if (dbResponse) {
        let hashedPass = dbResponse['password'];
        bcrypt.compare(password, hashedPass)
            .then(result => {
              if (result) {
                let userParsed = jsonParse(dbResponse) /*JSON.parse(JSON.stringify(userResponse));*/
                delete userParsed['password'];

                console.log("RESPONSE:", userParsed);
                res.status(200).json(userParsed);
              } else {
                res.status(200).json(dbResponse);
                //res.status(400).json({error: 'Error, wrong email / password combination..'});
              }
            });
      } else {
        res.status(400).json({error: 'Error, wrong email / password combination..'});
      ...
      res.status(400).json({error: 'Error, wrong email / password combination..' + e});
    }
  }
});
...
router.post('/create/caretaker', async function (req, res, next) {
  ...
  if (!isInputValid) {
    ...
    try {
      let response = await createNewCaretaker(fullName, parent_id, user_id);
      ...
      if (userResponse) {
        ...
        res.status(500).json({error: 'Error, creating new caretaker failed.. ' + response});
      }
    } catch (e) {
      res.status(500).json({error: 'Error, creating new caretaker failed.. ' + e});
    }
  ...
  if (!isInputValid) {
    ...
    try {
      ...
      if (users) {
        ...
        res.status(400).json({error: 'Error obtaining caretaker data..', response});
      }
    } catch (e) {
      res.status(400).json({error: 'Error obtaining caretaker data..', e});
    }
  }
});
  ...
  if (!isInputValid) {
    ...
    try {
      ...
      if (users) {
        ...
        res.status(400).json({error: 'Error updating user data..', response});
      }
    } catch (e) {
...
router.post('/user/update', async function (req, res, next) {
  ...
  if (!isInputValid) {
    ...
    try {
      let response = await updateUser(user_id, full_name, username, telephone);
  ...
  if (!isInputValid) {
    ...
    try {
      ...
      if (user) {
        ...
        res.status(400).json({error: 'Error creating user password request..', resetPasswordRequest});
      }
    } catch (e) {
      res.status(400).json({error: 'Error requesting user password reset..', e});
    }
  }
});
    ...
    try {
      ...
      bcrypt.hash(randomisePass, 3).then(async (hash) => {
        ...
        if (user) {
          ...
          let loginPageText =  "Do portala cenikov lahko dostopate na tej povezavi: https://klikni.si/\n\n"

          sendEmail("Pozdravljeni, geslo uspešno ponastavljeno", `Pozdravljeni, ${user.username}\n\n ${loginPageText} \nLepo pozdravljeni`, user.username);

          let userParsed = jsonParse(user) /*JSON.parse(JSON.stringify(userResponse));*/
          delete userParsed['password'];
          res.status(200).json(userParsed);
        } else {
          res.status(400).json({error: 'Error updating user password..', response});
        }
      });
    } catch (e) {
      res.status(400).json({error: 'Error updating user password..', e});
    }
  }
});
