...
async function requestPaymentIntent(req, res) {
	try {
	  ...
	  // Create a Payment Method to handle the transaction
	  let paymentIntent = await stripe.createPaymentIntent(Math.round(amount * 100), currency, req.user.stripe_customer_id, return_url, { user_id: req.user.user_id, type: "wallet_topup" });
	  res.status(200).json(paymentIntent);
	} catch (error) {
